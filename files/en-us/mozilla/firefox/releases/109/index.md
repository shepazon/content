...
- The {{HTMLElement("input/range", "range")}} form of the {{HTMLElement("input")}} element now supports [`list`](/en-US/docs/Web/HTML/Element/input/range#list) attribute which links via an id to a {{HTMLElement("datalist")}} to allow Firefox to display tick marks along the range.
